...
namespace JobChannel.DAL.UOW.Repositories.JobOfferRepositories
{
    internal class JobOfferRepository : IJobOfferRepository
    {
        ...
        public JobOfferRepository(IDbSession dbSession, ILogger<JobOfferRepository> logger) => _dbSession = dbSession;
        ...
        public async Task<IEnumerable<JobOffer>> GetAll(IReadOnlyDictionary<string, dynamic>? searchFields)
        {
            string query = @"SELECT jo.Id, jo.Title, jo.Description, jo.PublicationDate, jo.ModificationDate,
                            ...
                            c.Code, c.Population, d.Id, d.Name, d.Code,
            ...
            if (searchFields != null)
            {
                ...
                foreach (var field in searchFields)
                {
                    ...
                    query += field.Value switch
                    {
                        ...
                        {
                            "Page" => $"{(orderBy ? "" : " ORDER BY jo.PublicationDate DESC")} OFFSET {(field.Value - 1) * searchFields["Count"]} ROWS",
                            "Count" => $"{(orderBy ? "" : " ORDER BY jo.PublicationDate DESC")} FETCH NEXT {field.Value} ROWS ONLY",
            ...
            var jobOffers = await _dbSession.Connection.QueryAsync<JobOffer, Job, City, Department, Region, Contract, JobOffer>(query, (jobOffer, job, city, department, region, contract) =>
            {
                jobOffer.Job = job;
                jobOffer.City = city;
